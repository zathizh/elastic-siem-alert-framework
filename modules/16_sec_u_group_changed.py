...
ITEM_PATH_EXCLUSIONS = "exclusions/users/sec_universal_group.lst"
...
def main():
    ...
    estack.setRangeQuery(event_id=4755, period=PERIOD)
    ...
    if count > THRESHOLD:
        ...
        for record in hits:
            ...
            item = event_data['TargetUserName']
            _timestamp = datetime.strptime(source['@timestamp'], "%Y-%m-%dT%H:%M:%S.%fZ").strftime("%H:%M:%S")

            if item not in excluded_items:
                if args.debug:
                    print(item)
                artifacts.append([_timestamp, source['winlog']['computer_name'], item, event_data['SubjectUserName']])
                counter+=1
        ...
        if counter :
            ...
            mailbody = "{counter}/{count} security-enabled universal group change events were detected during last 5 minutes\n\n".format(counter=counter, count=count)
            em = EmailReport(subject=org + "Alert - A security-enabled universal group was changed [Excluding the defined exclusions]", body=mailbody, table=table)
            em.sendEmail()
